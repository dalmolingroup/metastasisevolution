...
```{r}
  ...
  cumulative_genes <- node_annotation %>%
    arrange(desc(root)) %>%
    dplyr::select(queryItem, root, clade_name) %>%
    unique() %>%
    group_by(-root, clade_name) %>%
  ...
  cumulative_bp <- node_annotation %>%
    arrange(desc(root)) %>%
    dplyr::select(Signature, root, clade_name) #%>%
    unique() %>%
    group_by(-root, Signature) %>%
    summarise(count_signatures = n(), .groups = "drop")

   combined_data <- cumulative_genes %>%
    full_join(cumulative_bp, by = "root") %>%
    pivot_wider(names_from = Signature, values_from = count_signatures, values_fill = list(count = 0)) %>%
     replace_na(list(
    `cell adhesion` = 0,
    `cell junction organization` = 0,
    `cellular extravasation` = 0,
    `epithelial to mesenchymal transition` = 0,
    `extracellular matrix organization` = 0,
    `regulation of metallopeptidase activity` = 0
  ))
    
    
  #   summarized_data <- combined_data %>%
  group_by(clade_name, Signature) %>%
  summarize(count = sum(cumulative_sum, na.rm = TRUE), .groups = 'drop')
    
  # Selecionar nomes únicos para o eixo x (sem duplicações)
  unique_clade_names <- unique(cumulative_data$clade_name)
  
  # Criar gráfico com preenchimento pelas Signatures (processos biológicos)
  ggplot(cumulative_bp, aes(x = root, y = count_signatures, fill = Signature)) +
    geom_bar(stat = "identity") +
    geom_text(aes(label = count_signatures), vjust = 2, size = 3, color = "white") +
    labs(x = NULL, y = NULL) +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1), plot.margin = margin(1, 1, 0, 1))
  
  ggplot(combined_data, aes(x = factor(clade_name, levels = unique_clade_names), y = cumulative_sum, fill = count_signatures)) +
  geom_bar(stat = "identity", position = "stack") +  # Usar position = "stack" para empilhar as barras
  geom_text(aes(label = cumulative_sum), position = position_stack(vjust = 0.5), size = 3, color = "white") +
