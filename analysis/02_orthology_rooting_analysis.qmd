...
get_string_ids <- function(genes_hgnc, species_id = "9606") {
  ...
  protein_info <- vroom::vroom(
    paste0(
      ...
      species_id, ".protein.info.v12.0.txt.gz"
    ),
    col_types = "ccc",
    delim = "\t",
    comment = "##"
  ...
  map_info <- protein_info %>%
    ...
    dplyr::select(stringId = "#string_protein_id", queryItem = preferred_name) %>%
    dplyr::distinct()
  ...
  aliases <- vroom::vroom(
    paste0(
      ...
      species_id, ".protein.aliases.v12.0.txt.gz"
    ),
    col_types = "ccc",
    delim = "\t",
    comment = "##",
...
string_eukaryotes <- vroom::vroom("https://stringdb-downloads.org/download/species.v12.0.txt") %>%
  subset(domain == "Eukaryotes")

## Table with Orthologous Groups and their proteins
download_if_missing("https://stringdb-static.org/download/COG.mappings.v12.0.txt.gz")

cogs <- fread(
  "../assets/COG.mappings.v12.0.txt.gz",
  header           = F,
  stringsAsFactors = F,
  skip             = 1,
  sep              = "\t",
...
cogs %<>% dplyr::select(-taxid.string_id) %>% 
  filter(taxid %in% string_eukaryotes[["#taxon_id"]])
gc()
...
```{r genebridge}
phyloTree <- readRDS("../assets/phylotreev12.rds")

## Run GeneBridge
ogr <- newBridge(ogdata=cogdata2, phyloTree=phyloTree, ogids = gene_cogs$cog_id, refsp="9606")
