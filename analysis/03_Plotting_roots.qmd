...
nodelist_filtered <- nodelist %>%
  ...
  mutate(node_size = case_when(
    clade_name %in% c("Metamonada", "Choanoflagellata", "Ctenophora", "Actinopterygii") ~ 50,
    TRUE ~ 35
  )) %>% 
  mutate(node_linecolor = case_when(
    queryItem %in% metastasis_genes$gene ~ "#6A0DAD", 
    TRUE ~ "#999999FF"
  )) %>%
    mutate(node_linewidth = case_when(
    queryItem %in% metastasis_genes$gene ~ 3, 
    TRUE ~ 1
    )) %>%
  filter(root >= 20)
```
...
```{r}
nodelist <- vroom::vroom(here("results/orthology_data/nodelist.csv"))
map_ids <- vroom::vroom(here("results/orthology_data/map_ids.csv"))
   ...
   for (process in processes) {
     for (clade in clades) {
      ...
      # N <- length(unique(metastasis_genes$external_gene_name))
       N <- length(unique(map_ids$stringId))
