...
metastasis_genes <- vroom::vroom("~/scratch/rstudio/tmp_tmp/metastasisevolution/results/metastasis_genes/metastasis_genes.csv", delim = ",")
nodelist <- vroom::vroom("~/scratch/rstudio/tmp_tmp/metastasisevolution/results/orthology_data/nodelist.csv")
...
## Ogs Hipergeometric test

```{r}
# Agregar para nível de COG
nodelist_cogs <- nodelist %>%
  group_by(cog_id, root, clade_name) %>%
  summarise(
    across(where(is.numeric), ~ as.integer(any(. == 1))),
    .groups = "drop"
  )

clades <- unique(nodelist_cogs$clade_name)
processes <- c(
  "cell adhesion", 
  "extracellular matrix organization", 
  "epithelial to mesenchymal transition", 
...
for (process in processes) {
  for (clade in clades) {
    ...
    # N = total de COGs únicos
    N <- length(unique(nodelist_cogs$cog_id))
    
    # M = total de COGs envolvidos nesse processo
    M <- sum(
      nodelist_cogs %>%
        dplyr::select(all_of(process)) %>%
        pull() == 1
    )
    
    # n = número de COGs no clado
    n <- sum(nodelist_cogs$clade_name == clade)
    
    # k = número de COGs no clado envolvidos no processo
    k <- sum(
      filter(nodelist_cogs, clade_name == clade)[[process]]
    )
    
    # Teste hipergeométrico
    p_valor <- 1 - phyper(k - 1, M, N - M, n)
    
    result[[paste(process, clade, sep = "_")]] <- 
      c(process, clade, k, M, N, n, p_valor)
  }
}

hipergeometric_OGs_df <- do.call(rbind, result) %>%
  as.data.frame() %>%
  setNames(c("Process", "clade_name", "k", "M", "N", "n", "p.val")) %>%
  arrange(p.val)

rownames(hipergeometric_OGs_df) <- NULL

hipergeometric_OGs_df <- hipergeometric_OGs_df %>%
  mutate(across(c("k", "M", "N", "n", "p.val"), as.numeric))

hipergeometric_OGs_df <- hipergeometric_OGs_df %>%
  mutate(p.adj = p.adjust(p.val, method = "fdr"))

head(hipergeometric_OGs_df)
```

```{r}
hipergeometric_OGs_df$p.adj <- as.numeric(hipergeometric_OGs_df$p.adj)
hipergeometric_OGs_df$p.val_color <- ifelse(hipergeometric_OGs_df$p.adj > 0.05, "gray", "red")
...
hipergeometric_OGs_df$clade_name <- factor(hipergeometric_OGs_df$clade_name, 
    levels = unique(hipergeometric_OGs_df$clade_name[order(hipergeometric_OGs_df$root)]))


hipergeometric_OGs_df$Process <- factor(hipergeometric_OGs_df$Process, 
                                    levels = c("extracellular matrix organization", 
                                               "epithelial to mesenchymal transition", 
                                               "cell junction organization",
                                               "regulation of metallopeptidase activity",
...
ggplot(hipergeometric_OGs_df, aes(x = Process, 
                              y = clade_name, 
                              size = as.numeric(n), 
                              color = p.val_color)) +
  geom_point() +
