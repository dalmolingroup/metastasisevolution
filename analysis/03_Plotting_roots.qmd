...
# baseline: subtrai o valor de Metamonada
calc_gaps2$acumulado <- calc_gaps2$acumulado - calc_gaps2$acumulado[1]

# >>>> REMOVE Metamonada do cálculo estatístico <<<<
gaps <- calc_gaps2$n[-1]
...
calc_gaps2$p_ajustado <- NA

calc_gaps2$modified_z_scores[-1] <- 0.6745 * (gaps - mediana) / mad
calc_gaps2$p_value[-1] <- 2 * pnorm(-abs(calc_gaps2$modified_z_scores[-1]))
calc_gaps2$p_ajustado[-1] <- p.adjust(calc_gaps2$p_value[-1], method = "fdr")

# filtra clados significativos (exclui metamonada)
signif_clades <- calc_gaps2 %>%
  filter(!is.na(p_ajustado),
         p_ajustado <= 0.05,
